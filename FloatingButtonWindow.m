...
- (void)handleRouteChange:(NSNotification *)note {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.keepAliveWanted && !self.audioPlayer.isPlaying) {
            IMLog(@"audio route changed, restarting keep-alive");
        }
        [self evaluateKeepAlive];
    });
}
...
- (void)keepAliveDidSucceed {
    if (self.audioFailureStreak > 0) {
        IMLog(@"keep-alive recovered after %ld failed attempt(s)", (long)self.audioFailureStreak);
    } else {
...
- (void)keepAliveDidFailWithError:(NSError *)error {
    ...
    if (self.audioFailureStreak == 1) {
        IMLog(@"keep-alive failed to start: %@. Will keep retrying while immortalized.",
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag {
    ...
    IMLog(@"silent player finished unexpectedly (success=%d), re-evaluating", flag);
    [self evaluateKeepAlive];
}

- (void)audioPlayerDecodeErrorDidOccur:(AVAudioPlayer *)player error:(NSError *)error {
    IMLog(@"silent player decode error: %@, rebuilding", error.localizedDescription);
