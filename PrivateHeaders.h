...
/* --- Single source of truth for the immortalized flag ---------------------
   NSUserDefaults is the persisted source, while main.m owns the atomic live
   cache used by the hot scene-hook path. FloatingButtonWindow always reads
   that cache instead of maintaining an independent toggle. These declarations
   keep both layers and the Darwin-notify name in one place. */
