...
static void installHook(void) {
    ...
    if (!targetClass) {
        IMLog(@"Immortalizer couldn't attach on this version of iOS — apps won't be kept awake");
        return;
    }

    Method originalMethod = class_getInstanceMethod(targetClass, selector);
    if (!originalMethod) {
        IMLog(@"Immortalizer couldn't attach (system method not found) — apps won't be kept awake");
        return;
    }
    ...
    IMLog(@"Immortalizer is active and watching for background attempts");
}
